...
const assignedPlanJs = projectFile('task-plan.js');
const builderUiJs = projectFile('builder-ui.js');
const courseBuilderHtml = projectFile('course-builder.html');
...
assert.match(builderHtml, /id="taskPlanSlug" type="hidden"/);
assert.match(builderHtml, /cmcTaskPlanBuilderShell/);
assert.match(builderHtml, /builder-ui\.js/);
assert.match(builderJs, /task-plan-preview\.html\?id=/);
assert.match(builderJs, /existingSlug \|\| slugify\(title\)/);
...
assert.match(previewJs, /cmcPlanSidebarProgress preview/);
assert.match(previewJs, /phaseProgress\.percent/);
assert.doesNotMatch(previewJs, /cmcPlanWorkspaceProgress preview/);
assert.match(previewJs, /cmcPlanPhasePager/);
assert.match(previewJs, /cmcPlanExpandCue/);
assert.doesNotMatch(previewJs, /<i>Details<\/i>/);
assert.match(assignedPlanJs, /cmcPlanSidebarProgress/);
assert.match(assignedPlanJs, /PHASE .* PROGRESS/);
assert.doesNotMatch(assignedPlanJs, /Overall plan progress/);
assert.match(assignedPlanJs, /cmcPlanPhasePager/);
assert.match(courseBuilderHtml, /cmcCourseBuilderShell/);
assert.match(courseBuilderHtml, /builder-ui\.js/);
assert.match(builderUiJs, /aria-expanded/);
...
assert.match(styles, /\.cmcPlanWorkspaceProgress/);
assert.match(styles, /\.cmcPlanPhasePager/);
